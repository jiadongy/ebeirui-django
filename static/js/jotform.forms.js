...
var JotForm = {
    ...
    paymentFields: ['control_2co', 'control_authnet', 'control_bluepay', 'control_bluesnap', 'control_boxpayment', 'control_braintree', 'control_cardconnect', 'control_chargify', 'control_clickbank', 'control_dwolla', 'control_echeck', 'control_eway', 'control_firstdata', 'control_googleco', 'control_moneris', 'control_onebip', 'control_pagseguro', 'control_payjunction', 'control_payment', 'control_paymill', 'control_paysafe', 'control_paypal', 'control_paypalexpress', 'control_paypalpro', 'control_payu', 'control_sofort', 'control_square', 'control_stripe', 'control_wepay', 'control_worldpay', 'control_payoneer'],
    isEncrypted: false,
    ...
    encryptAll: function (e, callback) {
        ...
        var selfSubmitFields = ["control_stripe", "control_braintree", "control_square", "control_eway", "control_bluepay", "control_wepay"];
    ...
    handleScreenshot: function () {
    ...
    }, dateLimits: {}, setCalendar: function (id, startOnMonday, limits, parent) {
        try {
    ...
    }, currentDateReadonly: function () {}, calendarClose: function (calendar) {
        ...
        return (typeof window.editModeFunction !== 'undefined') || window.location.pathname.match(/^\/edit\//) || window.location.pathname.match(/^\/\/edit/) || window.location.href.match(/mode=inlineEdit/) || window.location.href.match(/mode=submissionToPDF/);
    ...
    }, typeCache: {}, getInputType: function (id) {
        ...
        return date;
    }, hidePages: {}, checkCondition: function (condition) {
        var any = false,
            all = true;
        var filled;
                ...
                setTimeout(function (calc) {
                    ...
                    if (!matchForThisResult[condition.action[0].resultField + subfield]) {
                        try {
                            ...
                            if (toBeClearField in window.__antiLoopCache) {
                ...
                if (window.FORM_MODE == 'cardform') {
                    if (action.skipTo == 'end') {
                        ...
                        for (var i = nextQuestionIndex; i < allQuestions.length; i++) {
                            if (allQuestions[nextQuestionIndex].type === 'control_head') {
                                nextQuestionIndex++;
                            }
                        }
                        action.skipTo = allQuestions[nextQuestionIndex].id;
                        var next = sections.find(function (section) {
                            return section.id === 'id_' + action.skipTo;
                        });
                        if (next) {
                            JotForm.nextPage = sections.find(function (section) {
                                return section.id === 'id_' + action.skipTo;
                            });
                            JotForm.prevPage = sections.find(function (section) {
                                return section.className.indexOf('form-line-active') > -1;
                            });
                        }
                    }
                } else {
    ...
    requireField: function (qid, req) {
        ...
        $$('#id_' + qid + ' input, #id_' + qid + ' textarea, #id_' + qid + ' select').each(function (el) {
            if (el.id === 'coupon-input' || (el.type === 'hidden' && !el.up('.form-star-rating') && !el.hasClassName('form-widget')) || el.hasClassName('form-checkbox-other-input') || el.hasClassName('form-radio-other-input') || el.hasClassName('jfModal-input') || $A(['prefix', 'middle', 'suffix', 'addr_line2']).any(function (name) {
        ...
        try {
            if (!['text', 'email', 'textarea', 'calculation', 'combined', 'address', 'datetime', 'time', 'html', 'authnet', 'paypalpro', 'number', 'radio', 'checkbox', 'select', 'matrix', 'widget', 'signature', 'braintree', 'stripe', 'square', 'eway', 'bluepay', 'firstdata', 'chargify', 'echeck', 'payu', 'pagseguro', 'moneris', 'paypal', 'dwolla', 'bluesnap', 'paymentwall', 'payment', 'paypalexpress', 'payjunction', '2co', 'cardconnect', 'clickbank', 'onebip', 'worldpay', 'rating', 'hidden', 'file', 'other', 'mixed', 'sofort', 'payoneer', 'paysafe'].include(JotForm.calculationType(result))) return;
        } catch (e) {
            console.log(e);
        }
        var combinedObject = {};
    ...
    }, prices: {}, setCreditCardVisibility: function (show) {
        if (show) {
    ...
    }, discounts: {}, handleCoupon: function () {
        var $this = this;
    ...
    autoNext: function (id) {
        ...
        prev.observe("change", function (e) {
            if (!JotForm.isVisible(prev) || e.target.value === '') {
                return;
            }
        ...
        if (page > 2) JotForm.backStack = sections.splice(0, page - 1);
        JotForm.runAllCalculations(true);
        ...
        var validatorEvent = function (e) {
            setTimeout(function () {
                ...
                if ($this.lastFocus && (($this.lastFocus == input || $this.getContainer($this.lastFocus) != $this.getContainer(input)) || isLastRequiredField) && (!isPrefix) && (!(window.FORM_MODE === 'cardform' && inputContainer.dataset.type === 'control_fullname'))) {
                    input.validateInput();
        ...
        jQuery(_form).autoFill({
            ...
            onBeforeSave: function () {}, onSave: function () {}, onRelease: function () {}, onBeforeRestore: function () {}, onRestore: function (data) {
        ...
        maskValue = maskValue.replace('9', '\\9').replace('a', '\\a');
        var definitions = {
            "#": {
                validator: "[0-9]"
            }
        ...
        if (unmask) {
            ...
            }).attr('maskValue', maskValue).attr('placeholder', placeholder);
    ...
    }, initShoppingBag: function () {}, initProductPages: function () {}, initDonation: function () {}, customToQueryParams: function (text, separator) {
...
function onProductImageClicked(index) {
    ...
    if (window.getAllProperties && window.getAllProperties().form_products) {
        ...
        imageUrls = Array.prototype.map.call(document.querySelectorAll('.form-product-item img'), function (p) {
            return p.src
        });
    }
